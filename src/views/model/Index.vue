<template>
  <div>
    <a-layout style="height:100%; padding: 20px">
      ...
      <a-layout style="padding-left:20px;width: 40%">
        <a-layout-content
          :style="{ background: '#fff', padding: '20px', margin: 0, minHeight: '280px', overflow: 'auto'}"
        >
          <common-model v-bind:status="this.modelStatus" v-on:change="changeStatus"></common-model>
        </a-layout-content>
      </a-layout>
      <a-layout style="padding-left:20px;width: 60%">
        <a-layout-content
          :style="{ background: '#fff', padding: '20px', margin: 0, minHeight: '280px', overflow: 'auto'}"
        >
          <model-stopped v-show="modelStatus === 'stopped'"></model-stopped>
          <model-running v-show="modelStatus === 'running'"></model-running>
          <img v-show="modelStatus === 'finished' || modelStatus === 'cancelled'" src="http://172.22.0.4:6080/arcgis/rest/services/airport/MapServer/export?dpi=96&transparent=tr%20ue&format=png32&layers=show%3A0%2C1%2C2%2C3%2C4%2C5&bbox=-10658055.106393551%20%2C636386.9693250969%2C-10529946.646987546%2C763731.0584482494&bboxSR=102100&i%20mageSR=102100&size=838%2C833&f=image">
        </a-layout-content>
      </a-layout>
    </a-layout>
  </div>
